function simu_desc = main_uniform_queue_fct_with_purging(parameters,saved,draw,verbose)
...
parameters = no_choose_workers_with_purging(parameters);
...
parameters.t_feedback=[parameters.t_feedback parameters.t_feedback(end)+1]; %for simulation, we need this additionnal feedback time (which will not be useful as we will reach the max time before)
...
lambda_J_job = parameters.lambda_J_job; %arrival rate in task/time slot
...
index_service = ones(P,1); %up to which index service time have been collected
%service_time_collect=zeros(P,T); %collection of the service time
m1_est=zeros(P,length(t_feedback));%estimation of the service rate of the workers at each time step
m2_est=zeros(P,length(t_feedback));%estimation of the service rate of the workers at each time step
m1_est(:,1)=1./parameters.mu_est_init; %initialization
m2_est(:,1)=2./(parameters.mu_est_init.^2); %initialization
s_est=size(m1_est);
...
%[phi_init,~] = optimum_split(parameters.r_comp_scaled,r_comm_scaled,parameters.a,parameters.phi_l);
phi_init=ones(1,P)*1/P;
phi=phi_init;
%phi = parameters.r_comp_scaled./sum(parameters.r_comp_scaled);
    ...
    if t==tf_rec %if feedback time 
        ...
        if ~isempty(completed_tasks)
            ...
            for m=completed_tasks' %for each completed tasks
                ...
                %update m1 and m2
                %idx_serv = index_service(p);
                %m1_est(p,idx_serv+1) = m1_est(p,idx_serv)*(1-beta)+beta*t_serv;
                %m2_est(p,idx_serv+1) = m2_est(p,idx_serv)*(1-beta)+beta*t_serv^2;
                %index_service(p) = idx_serv+1;
                
                %update the last task the worker has processed
%                 worker_last(p)=m;
            end
...
%             for p=1:P
%                 if index_service(p)>0
%                     m1 = parameters.K*mean(service_time_collect(p,1:index_service(p)));
%                     var2 = parameters.K*var(service_time_collect(p,1:index_service(p)));
%                     %use memory in order to avoid bad estimation 
%                     m1_est(p,index_t_feedback) = m1_est(p,index_t_feedback-1)*(1-beta)+beta*m1;
%                     m2_est(p,index_t_feedback) = m2_est(p,index_t_feedback-1)*(1-beta)+beta*var2;
%                 else
%                     m1_est(p,index_t_feedback) = m1_est(p,index_t_feedback-1);
%                     m2_est(p,index_t_feedback) = m2_est(p,index_t_feedback-1);
%                 end
%                 %optimum_split
%                 
% 
%             end
%             linear_idx=sub2ind(s_est, 1:P, index_service');
%             m1=K*m1_est(linear_idx)';
%             r_comp_scaled_est = 1./(m1*lambda_J_job);
%             m2 = K^2*m2_est(linear_idx)';
%             a_est =0.5*lambda_J_job*m2./m1;
%             [phi,~] = optimum_split(r_comp_scaled_est,r_comm_scaled,a_est,phi_l);
        end
    ...
    for p=1:P
        if comp(p,1)<0 %if no task currently computed
            if ~isempty(Queue_comp{p})
                ...
                %if t_tasks <= t_out_vec(p) %if no time-out
                comp(p,2) = t+t_tasks;
                %else %time-out
                %    comp(p,2) = t+t_out_vec(p);
                %    comp(p,3) = 1;
                %end
...
%     end
    ...
    while ~jobs_all_received && t==job_desc(index_t_job_arrival).arrival_time
        ...
        n_tasks = round(phi*job_desc(j).rec_thres*Omega_forward); %number of tasks per worker
        d_tasks=ceil(job_desc(j).rec_thres*Omega_forward)-sum(n_tasks);
        if  d_tasks>0
            [~,I]=max(phi);
            n_tasks(I)=n_tasks(I)+d_tasks;
        end
        ...
        for p=1:P
            list_ID_tasks = index_task:index_task+n_tasks(p)-1; %ID of the tasks
            index_task=index_task+n_tasks(p);
            ...
            Queue_comm_in{p} = [Queue_comm_in{p} list_ID_tasks];
        end
...
results.index_service=index_service; 
results.m1_est=m1_est;
results.m2_est=m2_est;
