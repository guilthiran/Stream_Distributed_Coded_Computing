...
%         %idle_vec(niter,nn) = simu_results.analysis.percentage_idle_tot;
        ...
        [phi,~] = optimum_split(parameters.r_comp_scaled,parameters.r_comm_scaled,parameters.a,parameters.phi_l);
        phi=phi';
        Dcom = mean(phi./(parameters.r_comm_scaled*parameters.lambda_J));
        Dcomp = 1./parameters.lambda_J * mean((parameters.a.*phi.^2./(parameters.r_comp_scaled-phi)) + phi./parameters.r_comp_scaled);
        D = Dcom+Dcomp+parameters.Denc+parameters.Ddec;
        
        Dmean_vec(niter,nn)=D;
...
mean_delay = mean(Dmean_vec,1);
semilogy(Omega_vec,mean_delay);