function simu_desc = main_solution_queue_fct_gini_with_purging(parameters,saved,draw,verbose)
...
%     end
    ...
    for j = jmin:jmax
        ...
        %purge main queue also
         Lin = length(Queue_main);
            idx=1;
            while idx <=Lin 
                m=Queue_main(idx);
            ...
            Queue_main_length=length(Queue_main);
        end
    end
     %     
    %------------------------Queue comm out------------------------%
